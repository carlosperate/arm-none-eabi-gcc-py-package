...
<center><img src="https://github.com/user-attachments/assets/47db670d-b313-4f83-9616-418b32cadef6" alt="CLI animation installing and using compiler" width="75%"></center>
...
This is currently set up with the Test PyPI repository, but it has not
been released yet in the real PyPI.
...
GCC version and the `package_builder` versions together, where:
...
To install all the dependencies (recommended to use a virtual environment):
```bash
...
This scripts in the `package_builder` folder downloads the Arm GCC release,
extracts it within the `arm-none-eabi-gcc-toolchain` package folder, creates
the additional Python files required for the package, and builds it.
...
If the optional flags are used (both or neither must be specified), it will
...
The `package_builder` script will automatically create the source distribution
(`arm_none_eabi_gcc_toolchain-*.tar.gz`) in the `dist` folder as well.
...
[PEP 714](https://peps.python.org/pep-0714/) compliant static HTML Python
